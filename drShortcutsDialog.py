    ...
    def OnbtnGetKey(self, event):
        ...
        if alreadytaken > -1:
            if alreadytaken == 0:
                ...
                takenby = self.parent.DrScriptShortcutNamesAlpha[self.parent.ShortcutsArray[alreadytaken].index(newv)]
            else:
...
class drShortcutsDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, -1, ("Customize Shortcuts"), wx.DefaultPosition, (-1, -1), wx.DEFAULT_DIALOG_STYLE )
        ...
        #DrScript
        #prepaire for alphabetical display
        drscripshortcutssorted = zip(*sorted(zip(self.parent.DrScriptShortcutNames, self.parent.DrScriptShortcuts), key=lambda x: x[0]))
        self.DrScriptShortcutNamesAlpha = list(drscripshortcutssorted[0])
        self.ShortcutsArray[2] = list(drscripshortcutssorted[1])
        #end prepaire for alphabetical display
        ...
        self.boxShortcuts = drListBox(self, self.ID_SHORTCUTS, wx.DefaultPosition, (200, 200), self.parent.ShortcutNames)
    ...
    def OnbtnSave(self, event):
        self.update()
        #STC
        shortcutsfile = os.path.join(self.shortcutsdirectory, "stcshortcuts.dat")
        try:
        ...
        #DrScripts
        shortcutsfile = os.path.join(self.shortcutsdirectory, "drscript.shortcuts.dat")
        try:
        ...
        wx.MessageBox("To update the menu shortcuts, please restart DrPython", "Info")
    ...
    def OnList(self, event):
        ...
        if sel == 0:
            ...
            names = self.DrScriptShortcutNamesAlpha
    ...
    def update(self):
        ...
        #restore from alphabetical display
        i = 0
        for scriptname in self.DrScriptShortcutNamesAlpha:
            ind = self.parent.DrScriptShortcutNames.index(scriptname)
            self.parent.DrScriptShortcuts[ind] = self.ShortcutsArray[2][i]
            i += 1
        self.ShortcutsArray[2] = self.parent.DrScriptShortcuts
        #end restore from alphabetical display
