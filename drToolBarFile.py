...
def getToolBarList(datdirectory):
    toolbarfile = os.path.join(datdirectory, "toolbar.dat")
...
def getCustomBitmaps(datdirectory):
    filename = os.path.join(datdirectory, "toolbar.custom.icons.dat")
        ...
        if not bitmapNames[x] == "<Separator>":
            if bitmapNames[x] in customNames:
                ...
                if not os.path.exists(t):
                    t = os.path.join(defaultbitmapdirectory, iconsizestr, bitmapNames[x] + ".png")
            else:
                t = os.path.join(defaultbitmapdirectory, iconsizestr, bitmapNames[x] + ".png")
            if not os.path.exists(t):
                t = os.path.join(defaultbitmapdirectory, iconsizestr, "Default.png")
            fileArray.append(t)
        else:
...
def AddandReturn(frame, bitmapFileArray, IDNUM, count, title = ""):
    ...
    if title:         
        frame.toolbar.AddTool(IDNUM, title, wx.Bitmap(wx.Image(bitmapFileArray[count], wx.BITMAP_TYPE_PNG)))
        return IDNUM    
    frame.toolbar.AddTool(IDNUM, frame.ToolBarList[count], wx.Bitmap(wx.Image(bitmapFileArray[count], wx.BITMAP_TYPE_PNG)), wx.NullBitmap, wx.ITEM_NORMAL, frame.ToolBarList[count], "", None)
