...
class DrFrame(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        wx.Frame.__init__(self, parent, id, title, wx.DefaultPosition, (800, 600), name="DrPython")
        ...
        #Regex Line Endings:
        self.relewin = re.compile(r'\r\n', re.M)
        self.releunix = re.compile(r'[^\r]\n', re.M)
        self.relemac = re.compile(r'\re[^\n]', re.M)

        self.FormatMacReTarget = re.compile(r'((?<!\r)\n)|(\r\n)', re.M)
        self.FormatUnixReTarget = re.compile(r'(\r\n)|(\r(?!\n))', re.M)
        self.FormatWinReTarget = re.compile(r'((?<!\r)\n)|(\r(?!\n))', re.M)

        self.rechecksyntax = re.compile(r"line \d+", re.M)
        ...
        self.prefsfile = os.path.join(self.preferencesdirectory, "preferences.dat")
        ...
        if self.prefs.rememberwindowsizeandposition:
            sizeandpositionfile = os.path.join(self.datdirectory, "drpython.sizeandposition.dat")
            if os.path.exists(sizeandpositionfile):
                try:
                    f = open(sizeandpositionfile, 'r', encoding='utf-8')
                    text = f.read()
                    if text:
                    ...
                    self.ShowMessage("Error Loading Window Size.  The file '%s' may be corrupt." % sizeandpositionfile, "Error")
        ...
        icon.CopyFromBitmap(wx.Bitmap(wx.Image(os.path.join(self.bitmapdirectory, iconname), wx.BITMAP_TYPE_PNG)))
        self.SetIcon(icon)

        self.txtDocumentArray = []
        self.txtPromptArray = []

        self.mainpanel = drMainPanel(self, -1) #main panel
        ...
        self.documentnotebook = drDocNotebook(self.mainpanel.document, -1) #init doc notebook  
        self.promptnotebook = drPromptNotebook(self.mainpanel.prompt, -1) #init prompt notebook 
        
        self.documentnotebook.AddPage(drPanel(self.documentnotebook, self.ID_APP), "Untitled 1")        
        self.promptnotebook.AddPage(drPanel(self.promptnotebook, self.ID_APP), "Prompt") #add prompt notebook page
        ...
        self.retrailingwhitespace = re.compile(r'(?<=\S)[ \t]+$', re.MULTILINE)

        #Compile Regular Expressions for Open Import:
        self.reimport = re.compile(r'^\s*?import\s+?.*?$', re.M)
        self.refromimport = re.compile(r'^\s*?from\s+?.*?import.*?$', re.M)
        ...
        #Sizer
        self.bSizer.Add(self.mainpanel, 1, wx.EXPAND) #add main panel
    ...
    def Append_Menu(self, menu, id, s, LaunchesDialog = False, AmpersandAt = -1, absolutelabel=""):
        ...
        menuiconfile = os.path.join(self.bitmapdirectory, "16", s + ".png")
    ...
    def AddPluginIcon(self, name, location16, location24):
        ctbfile = os.path.join(self.datdirectory, "toolbar.custom.icons.dat")
        if not os.path.exists(ctbfile):
            f = open(ctbfile, 'w', encoding='utf-8')
            f.write('\n')
            f.close()
        f = open(ctbfile, 'r', encoding='utf-8')
        lines = f.read().split('\n')
        f.close()
        name = "<Plugin>:" + name
        f = open(os.path.join(self.datdirectory, "toolbar.custom.icons.dat"), 'w', encoding='utf-8')
        for line in lines:
            if line:
    ...
    def checkiffileisPython(self, filename):
        if self.refiletypeispy.search(filename) is not None:
            return True
        elif filename.find('.') in [0,-1]: #either like ".bashr" or "fstab"
            # mostly configuration or python file; python highlighting better than no highlighting
            return True
        else:
            return False
    ...
    def ExecuteWithPython(self, command = "", statustext = "", pythonargs="", pagetext="Python"):
        commandstring = " ".join([' -u', pythonargs, self.prefs.pythonargs, command])
        if self.PLATFORM_IS_WIN:
    ...
    def GetNewId(self):
        return 20000 + int(wx.Window.NewControlId(count=1)) # wx.NewId()
    ...
    def WriteUserPreferencesDirectoryFile(self):
        if not self.invokeuserpreferencespath:
            f = open(self.preferencesdirectoryfile, 'w', encoding='utf-8')
    ...
    def InitializeConstants(self):
        ...
        if not self.preferencesdirectory:
            ...
            if self.PLATFORM_IS_WIN:
                self.preferencesdirectoryprefix = "drpython"
            self.preferencesdirectory = os.path.join(self.preferencesdirectory, self.preferencesdirectoryprefix)
            self.preferencesdirectoryfile = os.path.join(self.preferencesdirectory, "userpreferencesdirectory.dat")
            ...
            f = open(self.preferencesdirectoryfile, 'r', encoding='utf-8')
            pth = f.read()
            f.close()
        ...
        if self.PLATFORM_IS_WIN:
            ...
            self.pythexec = "python3" #sys.prefix.replace('\\', '/') + "/python3"
    ...
    def LoadPlugins(self, plugins_file = ""):
        ...
        if plugins_file:
            ...
            pluginsfile = os.path.join(self.preferencesdirectory, "default.idx")
        if os.path.exists(pluginsfile):
            try:
                f = open(pluginsfile, 'r', encoding='utf-8')
    ...
    def LoadPopUpFile(self):
        ...
        popupfile = os.path.join(self.datdirectory, "popupmenu.dat")

        if os.path.exists(popupfile):
            try:
                f = open(popupfile, 'r', encoding='utf-8')
                line = f.readline()
                while len(line) > 0:
    ...
    def SetDrPythonDirectories(self):
        ...
        self.bitmapdirectory = os.path.join(self.programdirectory, "bitmaps")
    ...
    def LoadPreferences(self):
        ...
        if self.prefs.defaultencoding:
            importlib.reload(sys) # reload(sys)  this is needed because of wine and linux
            #sys.setdefaultencoding(self.prefs.defaultencoding)
            #wx.SetDefaultPyEncoding(self.prefs.defaultencoding)

    def LoadRecentFiles(self):
        filename = os.path.join(self.datdirectory, "recent_files.log")
    ...
    def LoadShortcuts(self, UseDefault = False):
        #Load STC Shortcuts
        stcshortcutsfile = os.path.join(self.shortcutsdirectory, "stcshortcuts.dat")
        ...
        shortcutsfile = os.path.join(self.shortcutsdirectory, "shortcuts.dat")
        ...
        drscriptsshortcutsfile = os.path.join(self.shortcutsdirectory, "drscript.shortcuts.dat")
    ...
    def OnPyDocAll(self, event):
        ...
        if dir:
            ...
            temp = open(tempname, 'w', encoding='utf-8')
            ...
            temp.close()
            temp = open(tempname, 'r', encoding='utf-8')
            msg = temp.read()
            temp.close()
            os.chdir(cwd)
    ...
    def OnClosePrompt(self, event):
        ...
        if len(self.txtPromptArray) > 1:
            ...
            self.promptnotebook.RemovePage(self.promptPosition)
    ...
    def OnCloseW(self, event):
        ...
        if self.prefs.rememberwindowsizeandposition:
            ...
            try:
                f = open(os.path.join(self.datdirectory, "drpython.sizeandposition.dat"), 'w', encoding='utf-8')
                x, y = self.GetSize()
    ...
    def OnEditBookmarks(self, event):
        ...
        d = drBookmarksDialog(self, os.path.join(self.datdirectory, "bookmarks.dat"))
        d.ShowModal()
        d.Destroy()
    ...
    def OnEnd(self, event):
        if self.txtPrompt.pid != -1:
            ...
            self.UpdateMenuAndToolbar()
            wx.Kill(self.txtPrompt.pid,wx.SIGKILL)
    ...
    def OnFindAndComplete(self, event):
        ...
        repre = re.compile(r"\(|\)|\[|\]|\{|\}|\<|\>|\.", re.IGNORECASE | re.MULTILINE)
        regex = re.compile(r"\w*\Z", re.IGNORECASE | re.MULTILINE)
    ...
    def OnNew(self, event):
        l = len(self.txtDocumentArray)
        unumbers = list(map(lambda x: x.untitlednumber, self.txtDocumentArray))
        unumbers.sort()
        x = 0
        last = 0
        while x < l:
    ...
    def OnRun(self, event):
        ...
        if self.checkiffileisPython (self.txtDocument.filename):
            ...
            if self.PLATFORM_IS_WIN:
                ...
                self.runcommand(("python3"+ ' ' + self.txtDocument.filename + ' '  + largs), "Running " + filen, filen)                #patch: [ 1366679 ] Goto Line Should Not Display At Top Of Window
        elif ext == ".lua":
    ...
    def OnSaveAs(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.reloaddocumentsmenu()
            #Refreshes the tab.
            x, y = self.documentnotebook.GetSize()
    ...
    def OnSavePrompt(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                cfile = open(pfilename, 'w', encoding='utf-8')
                cfile.write(ctext)
                cfile.close()
            except:
    ...
    def OpenFile(self, filename, OpenInNewTab, editrecentfiles = True, encoding="<Default Encoding>"):
        ...
        if not os.path.exists(filename):
            ...
            return
        try:
            cfile = open(filename, 'r', encoding='utf-8')
        except:
    ...
    def RemovePluginIcon(self, name):
        toolbarfile = os.path.join(self.datdirectory, "toolbar.custom.icons.dat")
        f = open(toolbarfile, 'r', encoding='utf-8')
        lines = f.read().split('\n')
        f.close()
        name = "<Plugin>:" + name
        f = open(toolbarfile, 'w', encoding='utf-8')
        for line in lines:
            if line:
                currentname = drPrefsFile.ExtractPreferenceFromText(line, "Name")
                if currentname != name:
    ...
    def RemoveTrailingWhitespace(self, docPos):
        ...
        if self.prefs.docremovetrailingwhitespace[self.txtDocument.filetype]:
            ...
            nr_lines = 0
            nr_clines = 0
            regex = re.compile(r'\s+' + eol, re.MULTILINE)
    ...
    def LoadDialogSizeAndPosition(self, dialog, dialogfile, defaultdir=""):
        if self.prefs.rememberdialogsizesandpositions:
            ...
            sizeposfile = os.path.join(defaultdir, dialogfile)
            try:
                if os.path.exists(sizeposfile):
                    f = open(sizeposfile, 'r', encoding='utf-8')
                    text = f.read()
                    f.close()
    ...
    def SaveDialogSizeAndPosition(self, dialog, dialogfile, defaultdir=""):
        if self.prefs.rememberdialogsizesandpositions:
            try:
                if not defaultdir:
                    defaultdir = self.datdirectory
                f = open(os.path.join(defaultdir, dialogfile), 'w', encoding='utf-8')
                x, y = dialog.GetSize()
                px, py = dialog.GetPosition()
                f.write(str(x) + '\n' + str(y) + '\n' + str(px) + '\n' + str(py))
                f.close()
            except:
    ...
    def SaveFile(self, docPos, IsSaveAs = False, encoding="FromText"):
        ...
        try:
            ...
            if self.PLATFORM_IS_GTK:
              if (not os.access(self.txtDocumentArray[docPos].filename, os.W_OK)) and \
                  ...
                  if answer == wx.YES:
                      ...
                      tmpfile = open(savetmpfile, 'w', encoding='utf-8')
                      tmpfile.write(ctext)
                      tmpfile.close()
            ...
            if not alreadysaved:
                cfile = open(self.txtDocumentArray[docPos].filename, 'w', encoding='utf-8')
                cfile.write(ctext)
                cfile.close()
    ...
    def setupdocumentsmenu(self):
        ...
        self.documentsmenu.AppendSubMenu(self.tabnavmenu, "Navigation")
        ...
        indextitles = list(map(_get_title, self.txtDocumentArray))
        sortedtitles = list(map(_x, indextitles))
...
#                    document.SourceBrowser = None
    ...
    def CreateMenus(self):
        ...
        self.filemenu.AAppend(self.ID_NEW, "New", False, 0)
        self.filemenu.AAppend(self.ID_OPEN, "Open", True, 0)
        self.filemenu.AAppend(self.ID_OPEN_IMPORTED_MODULE, "Open Imported Module", True)
        self.recentmenu = self.CreateRecentFileMenu()
        self.filemenu.AppendSubMenu(self.recentmenu, "Op&en Recent")
        self.filemenu.AAppend(self.ID_RELOAD, "Reload File", False, 0)
        self.filemenu.Append(self.ID_RESTORE_FROM_BACKUP, "Restore From Backup")
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_CLOSE, "Close", False, 0)
        self.filemenu.AppendSeparator()
        self.filemenu.Append(self.ID_CLEAR_RECENT, "Clear Recent File List")
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_SAVE, "Save", False, 0)
        self.filemenu.AAppend(self.ID_SAVE_AS, "Save As", True, 5)
        self.filemenu.AAppend(self.ID_SAVE_COPY, "Save A Copy", True, 2)
        self.filemenu.AAppend(self.ID_SAVE_PROMPT, "Save Prompt Output To File", True)
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_PRINT_SETUP, "Print Setup", True)
        self.filemenu.AAppend(self.ID_PRINT, "Print File", True, 0)
        self.filemenu.AAppend(self.ID_PRINTPROMPT, "Print Prompt", True)
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_EXIT, "Exit", False, 1)
        ...
        self.whitespacemenu.AAppend(self.ID_INDENT_REGION, "Indent", False, 0)
        self.whitespacemenu.AAppend(self.ID_DEDENT_REGION, "Dedent", False, 0)
        ...
        self.casemenu.AAppend(self.ID_UPPERCASE, "Uppercase", False, 0)
        self.casemenu.AAppend(self.ID_LOWERCASE, "Lowercase", False, 0)

        self.editmenu = drMenu(self)

        self.editmenu.AAppend(self.ID_UNDO, "Undo", False, 0)
        self.editmenu.AAppend(self.ID_REDO, "Redo", False, 1)
        ...
        self.editmenu.AppendSeparator()
        self.editmenu.AAppend(self.ID_INSERT_SEPARATOR, "Insert Separator", True)
        self.editmenu.AAppend(self.ID_INSERT_REGEX, "Insert Regular Expression", True)
        self.editmenu.AppendSeparator()
        self.editmenu.Append(self.ID_FIND_AND_COMPLETE, "Find And Complete")
        self.editmenu.AppendSeparator()
        self.editmenu.AppendSubMenu(self.commentmenu, "&Comments")
        self.editmenu.AppendSubMenu(self.whitespacemenu, "&Whitespace")
        self.editmenu.AppendSubMenu(self.casemenu, "Case")

        self.searchmenu = drMenu(self)
        self.searchmenu.AAppend(self.ID_FIND, "Find", True, 0)
        self.searchmenu.AAppend(self.ID_FIND_NEXT, "Find Next", False, 5)
        self.searchmenu.Append(self.ID_FIND_PREVIOUS, "Find Previous")
        self.searchmenu.AAppend(self.ID_REPLACE, "Replace", True, 0)

        self.foldmenu = drMenu(self)
        self.foldmenu.AAppend(self.ID_TOGGLE_FOLD, "Toggle Fold", False, 0)
        self.foldmenu.AAppend(self.ID_FOLD_ALL, "Fold All", False, 0)
        self.foldmenu.AAppend(self.ID_EXPAND_ALL, "Expand All", False, 0)
        ...
        self.gotomenu.AAppend(self.ID_GOTO_BLOCK_START, "Go To Block Start", False, -1, "Block Start")
        self.gotomenu.AAppend(self.ID_GOTO_BLOCK_END, "Go To Block End", False, -1, "Block End")
        self.gotomenu.AAppend(self.ID_GOTO_CLASS_START, "Go To Class Start", False, -1, "Class Start")
        self.gotomenu.AAppend(self.ID_GOTO_CLASS_END, "Go To Class End", False, -1, "Class End")
        self.gotomenu.AAppend(self.ID_GOTO_DEF_START, "Go To Def Start", False, -1, "Def Start")
        self.gotomenu.AAppend(self.ID_GOTO_DEF_END, "Go To Def End", False, -1, "Def End")

        self.viewmenu = drMenu(self)
        self.viewmenu.AAppend(self.ID_GOTO, "Go To", True, 0)
        self.viewmenu.AppendSubMenu(self.gotomenu, "Go To")
        self.viewmenu.AppendSeparator()
        self.viewmenu.AAppend(self.ID_ZOOM_IN, "Zoom In", False, 5)
        self.viewmenu.AAppend(self.ID_ZOOM_OUT, "Zoom Out", False, 5)
        self.viewmenu.AppendSeparator()
        self.viewmenu.AppendSubMenu(self.foldmenu, "&Folding")
        self.viewmenu.AppendSeparator()
        self.viewmenu.AppendSubMenu(self.viewinpanelmenu, "&View In Panel")
        self.viewmenu.AppendSeparator()
        self.viewmenu.AppendSubMenu(self.highlightmenu, "&Syntax Highlighting")
        self.viewmenu.AppendSeparator()
        self.viewmenu.Append(self.ID_TOGGLE_SOURCEBROWSER, "Toggle Source Browser")
        self.viewmenu.AAppend(self.ID_SOURCEBROWSER_GOTO, "Source Browser Go To", True)
        self.viewmenu.AppendSeparator()
        #fix bug someone refered in forum limodou 2004/04/20
        self.viewmenu.AAppend(self.ID_TOGGLE_VIEWWHITESPACE, "Toggle View Whitespace", False, 12)
        #end limodou
        ...
        self.programmenu.AAppend(self.ID_SET_ARGS, "Set Arguments", True)
        self.programmenu.AAppend(self.ID_PYTHON, "Python", False, -1, "Open a Python Interpreter")
        ...
        self.optionsmenu.AAppend(self.ID_PREFS, "Preferences", True, 0)
        self.optionsmenu.AAppend(self.ID_SHORTCUTS, "Customize Shortcuts", True, 0)
        self.optionsmenu.AAppend(self.ID_POPUP, "Customize Pop Up Menu", True)
        self.optionsmenu.AAppend(self.ID_CUSTOMIZE_TOOLBAR, "Customize ToolBar", True)
        self.optionsmenu.AppendSeparator()
        self.optionsmenu.AAppend(self.ID_EDIT_BOOKMARKS, "Edit Bookmarks", True, 0)
        self.optionsmenu.AAppend(self.ID_EDIT_SCRIPT_MENU, "Edit &Script Menu", True)
        self.optionsmenu.AppendSeparator()
        self.optionsmenu.AppendSubMenu(self.pluginsconfiguremenu, "C&onfigure Plugins")
        self.optionsmenu.AppendSubMenu(self.pluginsprefsmenu, "Pl&ugin Preferences")
        self.optionsmenu.AppendSubMenu(self.pluginsindexmenu, "&Load Plugin From Index")
        ...
        self.helpmenu.AppendSubMenu(self.pluginsaboutmenu, "About Plugin")
        self.helpmenu.AppendSeparator()
        self.helpmenu.AAppend(self.ID_HELP, "Help", True, 0, "DrPython &Help...")
        self.helpmenu.AppendSubMenu(self.pluginshelpmenu, "Plugin Help")
        self.helpmenu.AppendSeparator()
        self.helpmenu.AAppend(self.ID_PYTHON_DOCS, "View Python Docs", True)
        self.helpmenu.AAppend(self.ID_WXWIDGETS_DOCS, "View WxWidgets Docs", True)
        self.helpmenu.AAppend(self.ID_REHOWTO_DOCS, "View Regular Expression Howto", True)
    ...
    def promptDir(self, msg):
        ...
        d = wx.DirDialog(self, msg, style=wx.DD_DEFAULT_STYLE|wx.DD_NEW_DIR_BUTTON|wx.MAXIMIZE_BOX)
        dir = ""
        if d.ShowModal() == wx.ID_OK:
            dir = d.GetPath()
        d.Destroy()
...
class SysOutListener:
    ...
    def flush(self):
        # `write()` always opens and closes the file.  Nothing to flush here.
        pass
    ...
    def write(self, string):
        if len(string) > 1:
            ...
            if not os.path.exists(err_file):
                did_not_exist = True
            f=open(err_file, 'a', encoding='utf-8')
            if self.first:
                self.first = False
                if did_not_exist:
                    first = open(firsterr_file, 'w', encoding='utf-8')
                    first.close()
                f.write('\n\n' + time.ctime() + ':\n')
            f.write(string)
            f.flush()
...
class DrApp(wx.App):
    ...
    def OnIdle(self, event):
        ...
        if self.idlecnt == 2:
            ...
            if os.path.isfile(openfiletmp):
                f = open(openfiletmp, 'r', encoding='utf-8')
                filename = f.read()
                f.close()
                #print filename
    ...
    def OnInit(self):
        """Initializes the application and checks if the user wants to open a
        file in an existing instance.

        :todo: Get rid of the magic 101 ID in creating the frame.
        :todo: Maybe expand the *--s* option to more than one file or even
            add a remote interface so that commands like "open file" can be
            send to a running DrPython instance.
        """
        LOG.info('Starting up DrPython %s', DRPY_VER)
        atexit.register(LOG.info, 'Shutting down DrPython.')
        ...
        if get_option("--s") and self.instance.IsAnotherRunning():
            ...
            openfiletmp = os.path.join(os.path.expanduser('~'), "openfile.tmp")
            f = open(openfiletmp, 'w', encoding='utf-8')
