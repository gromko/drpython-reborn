...
class FileListCtrl(wx.ListView):
    def __init__(self, parent, id, fsize, fstyle):
        ...
        bDir = os.path.join(parent.parent.bitmapdirectory, "16")
        ...
        self.pywfilter = re.compile(r'.*\.pyw$')
        self.pyfilter = re.compile(r'.*\.py$')

        self.imagelist = wx.ImageList(16, 16)
        self.images = [wx.Bitmap(wx.Image(os.path.join(bDir, "folder.png"), wx.BITMAP_TYPE_PNG)), \
        wx.Bitmap(wx.Image(os.path.join(bDir, "py.png"), wx.BITMAP_TYPE_PNG)), \
        wx.Bitmap(wx.Image(os.path.join(bDir, "pyw.png"), wx.BITMAP_TYPE_PNG)), \
        wx.Bitmap(wx.Image(os.path.join(bDir, "unknown.png"), wx.BITMAP_TYPE_PNG)), \
        wx.Bitmap(wx.Image(os.path.join(bDir, "up.png"), wx.BITMAP_TYPE_PNG)), \
        wx.Bitmap(wx.Image(os.path.join(bDir, "down.png"), wx.BITMAP_TYPE_PNG))]
    ...
    def SetDirectoryTo(self, directory, filter="*", updatedirectories=True):
        ...
        try:
            ...
            x = 0
            while x < l:
                if os.path.isdir(os.path.join(directory, thelist[x])):
            ...
            for item in dirs:
                ...
                if additem:
                    ...
                    if self.IsDetailed:
                        ...
                        st = os.stat(os.path.join(directory, item))
...
class drFileDialog(wx.Dialog):
    def __init__(self, parent, title, wildcard, point=wx.DefaultPosition, size=wx.DefaultSize, IsASaveDialog=0, MultipleSelection=0, ShowRecentFiles=0):
        ...
        bDir = os.path.join(self.parent.bitmapdirectory, "24")

        self.btnDisplayList = wx.BitmapButton(self, self.ID_DISPLAY_LIST, wx.Bitmap(wx.Image(os.path.join(bDir, "list.png"), wx.BITMAP_TYPE_PNG)))
        self.btnDisplayDetailed = wx.BitmapButton(self, self.ID_DISPLAY_DETAILED, wx.Bitmap(wx.Image(os.path.join(bDir, "detailed.png"), wx.BITMAP_TYPE_PNG)))
        self.btnBack = wx.BitmapButton(self, self.ID_BACK, wx.Bitmap(wx.Image(os.path.join(bDir, "back.png"), wx.BITMAP_TYPE_PNG)))
        self.btnForward = wx.BitmapButton(self, self.ID_FORWARD, wx.Bitmap(wx.Image(os.path.join(bDir, "forward.png"), wx.BITMAP_TYPE_PNG)))
        self.btnUp = wx.BitmapButton(self, self.ID_UP, wx.Bitmap(wx.Image(os.path.join(bDir, "up.png"), wx.BITMAP_TYPE_PNG)))
        self.btnHome = wx.BitmapButton(self, self.ID_HOME, wx.Bitmap(wx.Image(os.path.join(bDir, "home.png"), wx.BITMAP_TYPE_PNG)))
        self.btnRefresh = wx.BitmapButton(self, self.ID_REFRESH, wx.Bitmap(wx.Image(os.path.join(bDir, "Reload File.png"), wx.BITMAP_TYPE_PNG)))
        self.btnBookmarks = wx.BitmapButton(self, self.ID_BOOKMARKS, wx.Bitmap(wx.Image(os.path.join(bDir, "bookmarks.png"), wx.BITMAP_TYPE_PNG)))
        self.btnDir = wx.BitmapButton(self, self.ID_BROWSE, wx.Bitmap(wx.Image(os.path.join(bDir, "folder.png"), wx.BITMAP_TYPE_PNG)))
        self.btnNewDirectory = wx.BitmapButton(self, self.ID_NEW_DIRECTORY, wx.Bitmap(wx.Image(os.path.join(bDir, "new directory.png"), wx.BITMAP_TYPE_PNG)))
        self.btnDeleteFile = wx.BitmapButton(self, self.ID_DELETE_FILE, wx.Bitmap(wx.Image(os.path.join(bDir, "Delete.png"), wx.BITMAP_TYPE_PNG)))
    ...
    def OnAddBookmark(self, event):
        try:
            bookfile = os.path.join(self.parent.datdirectory, "bookmarks.dat")
    ...
    def OnbtnNewDirectory(self, event):
        ...
        if answer == wx.ID_OK:
            if v:
                os.mkdir(os.path.join(self.lstFiles.GetDirectory(), v))
                self.OnbtnRefresh(None)

    def OnbtnOk(self, event):
        filename = self.cboFile.GetValue()
        if not os.path.isabs(filename):
            filename = os.path.join(self.txtDirectory.GetValue(), filename)
        if filename.find(".lnk") > -1:
            filename = self.HandleLnk(filename)
            if filename is None:
                return
    ...
    def OnFileActivated(self, event):
        ...
        filename = os.path.join(self.lstFiles.GetDirectory(), seltext)
        if filename.find(".lnk") > -1:
            filename = self.HandleLnk(filename)
            if filename is None:
                return
