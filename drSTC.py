...
class DrStyledTextControl(wx.stc.StyledTextCtrl):
    def __init__(self, parent, id, grandparent):
        ...
        self.retab = re.compile(r'^\t+', re.MULTILINE)
        self.respaces = re.compile(r'^ +', re.MULTILINE)
        self.remixedoutright = re.compile(r'(^\t+ )|(^ +\t)', re.MULTILINE)

        self.indentationtype = 2

        self.renonwhitespace = re.compile(r'\S')
    ...
    def OnPopUp(self, event):
        ...
        if event.ControlDown():
            ...
            dlg = drAboutDialog.drAboutDialog(self.grandparent, DRPY_VER)
